...
import { desserts, dishes, drinks } from "@/constants/data/dishes";
...
const Recommendations = () => {
  ...
  return (
    <div className="flex-vertical-center items-center px-5 py-5 w-full overflow-auto scroll-none">
      <h2 className="font-heading text-heading2 font-medium text-[22px] tracking-[-0.5px] leading-7.5 max-w-82 text-neutral-800 dark:text-white md:hidden text-center">
        We think you might enjoy these specially selected dishes
      </h2>

      {/* mobile selection bar */}
      <div className="flex items-center justify-between py-5 h-23.5 w-full max-w-11/12 md:hidden">
      ...
      <div className="md:flex items-center justify-between px-6 py-5 rounded-[20px] bg-white dark:bg-neutral-700 card-shadow h-23.5 w-full hidden  max-w-265">
      ...
      <div
        ...
        } gap-8 w-full overflow-auto scroll-none py-8 max-w-274`}
      >
        {list.map((item, index) => (
          <DishCard key={index} dish={item} viewMode={viewMode} />
        ))}
      </div>
    </div>
  );
