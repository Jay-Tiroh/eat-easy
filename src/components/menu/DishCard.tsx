...
import { DishCardProps } from "@/constants/types";
import { useSheet } from "@/store/sheetStore";
import { Plus, Star } from "lucide-react"; // Assuming you use lucide-react or similar for icons
import Image from "next/image";

export function DishCard({ dish, viewMode }: DishCardProps) {
  const { toggleSheet, setSheetContent } = useSheet();
  const handleClick = () => {
    setSheetContent(<DishSheet dish={dish} />);
    toggleSheet();
  };
  return viewMode === "list" ? (
    <ListCard dish={dish} handleClick={handleClick} />
  ) : (
    <GridCard dish={dish} handleClick={handleClick} />
  );
}

const ListCard = ({ dish, handleClick }: DishCardProps) => {
  return (
    <div className="flex items-center p-4 bg-white dark:bg-neutral-700 rounded-2xl shadow-sm card-shadow hover:shadow-md transition-shadow duration-300 w-full  max-w-11/12   h-33">
      {/* Image Container */}
...
const GridCard = ({ dish, handleClick }: DishCardProps) => {
  return (
